...
const Grades = ({grades}) => {
  const courseIds = [...Extension.state.courseIds, ...Extension.state.hiddenCourseIds];

  return (
    <Card title={"Grades"} id={"dashboard-grades"}>
      <table>
        <thead>
        <tr>
          <th>Name</th>
          ...
          <tr key={courseId}>
            <td>
              <a href={`/courses/${courseId}/grades`}>
                {grades[courseId].name}
              </a>
            </td>
            <td className={"assignments-score"}>
              <div>{grades[courseId].score}%</div>
            </td>
          </tr>
        )) : <></>}
        </tbody>
